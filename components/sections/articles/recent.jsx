...
export default function Recent({ mediumFeed }) {

	const articles = mediumFeed.items
	const thumbnail = mediumFeed.image
		...
		<Section classProp="borderBottom">
			<Container spacing={'verticalXXXXLrg'}>
				...
				<section className={css.projects}>
					{
						articles.map(({ title, published, link, author, category: categories }, index) => {
							const date = new Date(published).toDateString()
							return (
								<>
									<article key={index} className={css.project}>
										{/* <span className={css.featuredImage}>
											<Image src={thumbnail} alt="Article thumbnail" layout="responsive" width={500} height={300} />
										</span> */}
